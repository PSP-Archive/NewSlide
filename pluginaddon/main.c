...
PSP_MODULE_INFO("NewSlideWidgets", PSP_MODULE_USER, 1, 1);
PSP_MAIN_THREAD_ATTR(THREAD_ATTR_USER);

unsigned int CheckKeyPressKernel();
int pad;
int AddOptions = 0;
int InterfaceMode = PSP_UTILITY_HTMLVIEWER_INTERFACEMODE_NONE;
...
void htmlViewerInit(char *url)
{
	...
	params.numtabs = 1;
	params.cookiemode = PSP_UTILITY_HTMLVIEWER_COOKIEMODE_ENABLED;
	params.homeurl = url;
	params.textsize = PSP_UTILITY_HTMLVIEWER_TEXTSIZE_NORMAL;
	params.displaymode = PSP_UTILITY_HTMLVIEWER_DISPLAYMODE_SMART_FIT;
	params.options = PSP_UTILITY_HTMLVIEWER_DISABLE_STARTUP_LIMITS+AddOptions;
...
int main(int argc, char *argv[])
{
	...
	pspSdkLoadStartModule("ef0:/NEWSLIDE/NewSlideBridge.prx", PSP_MEMORY_PARTITION_KERNEL);
	
	setupGu();
	netInit();
	
	const char inifile[] = "ef0:/NEWSLIDE/settings.ini";
	
	//Get settings from file
	char url[100];
	ini_gets("NewSlideWidgets", "DefaultUrl", "file://ef0:/NEWSLIDE/widgets.htm", url, 100, inifile);
	int exit_on_open = ini_getl("NewSlideWidgets", "ExitOnSlide", 1, inifile);
	int flash = ini_getl("NewSlideWidgets", "EnableFlash", 0, inifile);
	int interface = ini_getl("NewSlideWidgets", "EnableChrome", 0, inifile);
	int cursor = ini_getl("NewSlideWidgets", "EnableCursor", 0, inifile);
	
	if (flash == 1){AddOptions = AddOptions + PSP_UTILITY_HTMLVIEWER_ENABLE_FLASH;}
	if (cursor == 0){AddOptions = AddOptions + PSP_UTILITY_HTMLVIEWER_DISABLE_CURSOR;}
	if (interface == 1){InterfaceMode = PSP_UTILITY_HTMLVIEWER_INTERFACEMODE_FULL;}
	...
	while(updateHtmlViewer())
	{
		sceDisplayWaitVblankStart();
		sceGuSwapBuffers();
		if ((pad == PSP_CTRL_SLIDEOPEN)||(pad == PSP_CTRL_SLIDEOPEN +PSP_CTRL_WLAN_UP)||(pad == PSP_CTRL_SLIDEOPEN+PSP_CTRL_MS)||(pad == PSP_CTRL_SLIDEOPEN+PSP_CTRL_WLAN_UP+PSP_CTRL_MS)){
			if (exit_on_open == 1){
				sceKernelExitGame();
			}
		}
